...
namespace SGL {
    ...
    public:
        ...
        function(std::function<Ret(Args...)> func, const state* s) : m_func(get_function_impl(func, s, std::make_index_sequence<sizeof...(Args)>{})) {
        }
    ...
    protected:
        ...
        static constexpr decltype(auto) get_function_impl(std::function<Ret(Args...)> func, const state* s, std::index_sequence<N...>) {
            return [func, s](std::initializer_list<std::reference_wrapper<value>> args) -> value {
                ...
                else return value(func((std::data(args)[N]).get().get<Args>() ...), s->get_value_type<Ret>());
            };
        }
